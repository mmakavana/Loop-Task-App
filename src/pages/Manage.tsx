          ...
          <ul className="space-y-1">
            {s.children.map(c=>(
              <li key={c.id} className="flex items-center gap-2">
                <span className="text-2xl">{c.avatarEmoji}</span><span>{c.name}</span>
              </li>
            ))}
        ...
        <div className="card space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2 flex gap-2">
              <button className="btn-outline" onClick={()=> useStore.getState().setPIN(pin,hint,rq,ra)}>Save PIN & Recovery</button>
              <button className="btn-ghost" onClick={()=> useStore.getState().relock()}>Lock Now</button>
            </div>
          </div>
        </div>
      </div>
